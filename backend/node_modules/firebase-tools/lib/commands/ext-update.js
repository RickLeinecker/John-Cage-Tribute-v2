...
exports.default = new command_1.Command("ext:update <extensionInstanceId> [updateSource]")
    ...
    .action((instanceId, updateSource, options) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let newSourceName;
        let published = false;
        const existingSourceOrigin = yield updateHelper_1.getExistingSourceOrigin(projectId, instanceId, existingSpec.name, existingSource);
        const newSourceOrigin = yield extensionsHelper_1.getSourceOrigin(updateSource);
        ...
        switch (newSourceOrigin) {
            case extensionsHelper_1.SourceOrigin.LOCAL:
                if (previews_1.previews.extdev) {
                    newSourceName = yield updateHelper_1.updateFromLocalSource(projectId, instanceId, updateSource, existingSpec, existingSource);
                    break;
                }
            case extensionsHelper_1.SourceOrigin.URL:
                if (previews_1.previews.extdev) {
                    newSourceName = yield updateHelper_1.updateFromUrlSource(projectId, instanceId, updateSource, existingSpec, existingSource);
                    break;
                }
            case extensionsHelper_1.SourceOrigin.OFFICIAL_EXTENSION_VERSION:
                newSourceName = yield updateHelper_1.updateToVersionFromRegistry(projectId, instanceId, existingSpec, existingSource, updateSource);
                break;
            case extensionsHelper_1.SourceOrigin.OFFICIAL_EXTENSION:
                newSourceName = yield updateHelper_1.updateFromRegistry(projectId, instanceId, existingSpec, existingSource);
                break;
            case extensionsHelper_1.SourceOrigin.PUBLISHED_EXTENSION_VERSION:
                if (previews_1.previews.extdev) {
                    newSourceName = yield updateHelper_1.updateToVersionFromPublisherSource(projectId, instanceId, updateSource, existingSpec, existingSource);
                    published = true;
                    break;
                }
            case extensionsHelper_1.SourceOrigin.PUBLISHED_EXTENSION:
                if (previews_1.previews.extdev) {
                    newSourceName = yield updateHelper_1.updateFromPublisherSource(projectId, instanceId, updateSource, existingSpec, existingSource);
                    published = true;
                    break;
                }
            default:
        ...
        const newSource = yield extensionsApi.getSource(newSourceName);
        ...
        yield updateHelper_1.displayChanges(existingSpec, newSpec, published);
        if (newSpec.billingRequired) {
            const enabled = yield checkProjectBilling_1.isBillingEnabled(projectId);
            if (!enabled) {
                yield billingMigrationHelper_1.displayNode10UpdateBillingNotice(existingSpec, newSpec, false);
                yield checkProjectBilling_1.enableBilling(projectId, instanceId);
            }
            else {
                yield billingMigrationHelper_1.displayNode10UpdateBillingNotice(existingSpec, newSpec, true);
            }
        }
        const newParams = yield paramHelper.promptForNewParams(existingSpec, newSpec, existingParams, projectId);
        spinner.start();
        const updateOptions = {
            projectId,
            instanceId,
